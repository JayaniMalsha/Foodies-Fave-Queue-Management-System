...
public class HelloApplication extends Application   {
    ...
    private static FoodQueue cashier1=new FoodQueue(2);
    private static FoodQueue cashier2=new FoodQueue(3);
    private static FoodQueue cashier3=new FoodQueue(5);
    ...
    private static void userChoise(String input){
        switch (input) {
            case "VFQ":
            case "100":
                viewAllQueues();
    ...
    private static void viewEmptyQueues() {
        ...
        for (int i = 0; i < 5; i++) {
            System.out.print("    ");
           if(cashier1.isQueueEmpty() && i<2){
               System.out.print("X ");
           }else {
               System.out.print("  ");
           }

            if(cashier2.isQueueEmpty() && i<3){
                System.out.print("X ");
            }else {
                System.out.print("  ");
            }
            if(cashier3.isQueueEmpty()){
                System.out.print("X ");
            }else {
                System.out.print("  ");
            }
    ...
    public static FoodQueue findSmallestQueue(FoodQueue queue1, FoodQueue queue2, FoodQueue queue3) {
        int size1 = queue1.getQueueSize();
        int size2 = queue2.getQueueSize();
        int size3 = queue3.getQueueSize();

        boolean queue1full=queue1.isQueueFull();
        boolean queue2full=queue2.isQueueFull();


        if ((size1 <= size2 && size1 <= size3) && !queue1full) {
            return queue1;
        } else if ((size2 <= size1 && size2 <= size3) && !queue2full) {
            return queue2;
        } else {
            return queue3;
        }
    ...
    private static void addCustomerToCashier() {
        String fname = stringInput("Please Enter your first name: ");
        ...
        FoodQueue smallestQueue = findSmallestQueue(cashier1,cashier2,cashier3);

        if (!smallestQueue.isQueueFull()) {
            smallestQueue.addCustomer(customer);
            System.out.println("Customer added to Queue " + (cashier1 == smallestQueue ? "1" : (cashier2 == smallestQueue ? "2" : "3")));
        } else {
            boolean addToWaitingQueue = false;
            while (!addToWaitingQueue) {
                String answer = stringInput("We're sorry, all the queues are full. Do you wish to be added to the waiting queue? (Y/N): ");
                if (answer.equalsIgnoreCase("Y")) {
                    ...
                    addToWaitingQueue = true;
                    // Handle the case where the customer does not want to be added to the waiting queue
                } else {
    ...
    private static void removeCustomerFromCashier() {
        ...
        try{
            if (cashierNumber == 1) {
                ...
                cashier1.removeCoustomer(customerNumber);
                cashier1.addCustomer(waitingQueue.get());
            } else if (cashierNumber == 2) {
                customerNumber = intInput("Enter the customer number (1, 2,3): ", 1, 3);
                cashier2.removeCoustomer(customerNumber);
                cashier2.addCustomer(waitingQueue.get());
            } else {
                customerNumber = intInput("Enter the customer number (1, 2,3,4,5): ", 1, 5);
                cashier3.removeCoustomer(customerNumber);
                cashier3.addCustomer(waitingQueue.get());
            }
        }catch (IndexOutOfBoundsException e){
            System.out.println("Customer removed from cashier");
        }
    ...
    private static void readProgramDataFromFile() {
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    // Add the customer to the corresponding cashier's queue based on the loop counter i
                    switch (i) {
                        case 0:
